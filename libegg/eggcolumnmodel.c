...
static GType
egg_column_model_get_column_type (GtkTreeModel *tree_model, gint index)
{
	switch (index) {
	case 0:
...
static gboolean
egg_column_model_get_iter (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreePath *path)
{
...
static GtkTreePath *
egg_column_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	EggColumnModel *column_model = (EggColumnModel *)tree_model;
	GtkTreePath *path;
	gint i = 0;
...
static gboolean
egg_column_model_iter_has_child (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	return FALSE;
}

static void
egg_column_model_get_value (GtkTreeModel *tree_model, GtkTreeIter *iter, gint column, GValue *value)
{
	switch (column) {
	case 0:
		g_value_init (value, G_TYPE_BOOLEAN);
		g_value_set_boolean (value, gtk_tree_view_column_get_visible (GTK_TREE_VIEW_COLUMN (((GList *)iter->user_data)->data)));
		break;
	case 1:
...
static gboolean
egg_column_model_iter_next (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	EggColumnModel *column_model = (EggColumnModel *)tree_model;

	g_return_val_if_fail (column_model->stamp == iter->stamp, FALSE);
...
static gint
egg_column_model_iter_n_children (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	if (iter)
		return 0;

	return g_list_length (((EggColumnModel *)tree_model)->columns);
}

static gboolean
egg_column_model_iter_nth_child (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent, gint n)
{
	EggColumnModel *column_model = (EggColumnModel *)tree_model;
...
static gboolean
egg_column_model_row_drop_possible (GtkTreeDragDest *drag_dest, GtkTreeModel *src_model, GtkTreePath *src_path, GtkTreePath *dest_path)
{
	gint *indices;
...
egg_column_model_finalize (GObject *object)
{
	...
	g_signal_handlers_disconnect_by_func (G_OBJECT (model->tree_view), update_columns, model);
...
egg_column_model_get_type (void)
{
	...
	if (!object_type) {
		...
		object_type = g_type_register_static (G_TYPE_OBJECT, "EggColumnModel", &object_info, 0);
...
static void
update_columns (GtkTreeView *tree_view, EggColumnModel *column_model)
{
...
egg_column_model_new (GtkTreeView *tree_view)
{
	...
	model = g_object_new (egg_column_model_get_type (), NULL);
...
egg_column_model_set_column_visible (EggColumnModel *model, GtkTreeIter *iter, gboolean visible)
{
	...
	column = ((GList *)iter->user_data)->data;
...
egg_column_model_move_up_column (EggColumnModel *model, GtkTreeIter *iter)
{
	...
	column = node->prev->data;
	base_column = node->data;
