                            ...
                            GtkRequisition *requisition)
{
  ...
  for (i = cellview->cell_list; i; i = i->next)
	...
	continue;
      ...
      if (!first_cell)
	requisition->width += cellview->spacing;
...
egg_cell_view_clear (EggCellView *cellview)
{
  ...
  for (list = cellview->cell_list; list; list = list->next)
    {
      EggCellViewCellInfo *info = (EggCellViewCellInfo *)list->data;
...
egg_cell_view_set_cell_data (EggCellView *cellview)
{
  ...
  path = gtk_tree_row_reference_get_path (cellview->displayed_row);
  gtk_tree_model_get_iter (cellview->model, &iter, path);
