...
#define INITIAL_ALLOCATION_MODE EGG_WRAP_ALLOCATE_HOMOGENEOUS
#define INITIAL_SPREADING       EGG_WRAP_BOX_SPREAD_START
#define INITIAL_MINIMUM_LENGTH  3
#define INITIAL_HSPACING        2
#define INITIAL_VSPACING        2

static EggWrapBox    *the_wrapbox       = NULL;
...
static void
populate_wrapbox_simple (EggWrapBox *wrapbox)
{
  ...
  for (i = 0; i < 30; i++)
    {
      ...
      egg_wrap_box_insert_child (EGG_WRAP_BOX (wrapbox), frame, -1,
                                 (items_xexpand ? EGG_WRAP_BOX_H_EXPAND : 0) |
                                 (items_yexpand ? EGG_WRAP_BOX_V_EXPAND : 0)); 
...
static void
populate_wrapbox_stock (EggWrapBox *wrapbox)
{
  GtkWidget *widget;
  ...
  for (i = 0, l = stock_ids; i < 30 && l != NULL; i++, l = l->next)
    {
      ...
      egg_wrap_box_insert_child (EGG_WRAP_BOX (wrapbox), widget, -1,
                                 (items_xexpand ? EGG_WRAP_BOX_H_EXPAND : 0) |
                                 (items_yexpand ? EGG_WRAP_BOX_V_EXPAND : 0));
    }
}
...
static void 
populate_items (EggWrapBox *wrapbox)
{
  GList *children, *l;
...
mode_changed (GtkComboBox   *box,
              EggWrapBox    *wrapbox)
{
  EggWrapAllocationMode mode = gtk_combo_box_get_active (box);
  
  egg_wrap_box_set_allocation_mode (wrapbox, mode);
}
...
horizontal_spreading_changed (GtkComboBox   *box,
			      EggWrapBox    *wrapbox)
{
  EggWrapBoxSpreading spreading = gtk_combo_box_get_active (box);
  
  egg_wrap_box_set_horizontal_spreading (wrapbox, spreading);
}
			    ...
			    EggWrapBox    *wrapbox)
{
  ...
  egg_wrap_box_set_vertical_spreading (wrapbox, spreading);
}
                     ...
                     EggWrapBox *wrapbox)
{
  ...
  egg_wrap_box_set_minimum_line_children (wrapbox, length);
}
                 ...
                 gpointer       data)
{
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    egg_wrap_box_set_horizontal_spacing (the_wrapbox, state);
  else
    egg_wrap_box_set_vertical_spacing (the_wrapbox, state);
}
...
create_window (void)
{
  ...
  wrapbox = egg_wrap_box_new (INITIAL_ALLOCATION_MODE, INITIAL_SPREADING,
			      INITIAL_SPREADING, INITIAL_HSPACING, INITIAL_VSPACING);
  the_wrapbox = (EggWrapBox *)wrapbox;
  egg_wrap_box_set_minimum_line_children (EGG_WRAP_BOX (wrapbox), INITIAL_MINIMUM_LENGTH);
  ...
  populate_items (EGG_WRAP_BOX (wrapbox));
